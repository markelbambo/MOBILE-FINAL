...
function userInformation2(){
	...
	if(globalUserName.length > 9){
		userName = globalUserName.substring(0,5)+'...';
	}
	$('#spanUserLabel').empty().append(userName);
	...
	if(type == "hubspoke"){
		...
		for(var i = 0 ; i < dev.length; i++){
			...
			for(x = 0 ; x < dev.length; x++){
				...
				var index = checkArr2.indexOf(dev[x].HostName);
				if(dev[i].HostName != dev[x].HostName && checkArr[index] != dev[i].HostName && checkArr2[index] != dev[i].HostName){
					var lineInfo = checkSamePort(dev[i],dev[x]);
					checkArr.push(dev[x].HostName);
					checkArr2.push(dev[i].HostName);
					console.log('>>>>>>>>>>',lineInfo);
					if(lineInfo.length == 1){
						msg.push(lineInfo.toString());
					}
					var name = lineInfo[0]+"_"+i;
 ...
 */
var counterPort=0;
function checkAvailablePorts(msg){
	if(msg.length > 0){
		var name = msg;
		var ports = name[0];
		for(var i = 1; i < name.length; i++){
			if(name.length == 2){
				...
				if(i < name.length - 1){
					...
					ports += ' and ' + name[name.length - 1];
				}
			}
		}
		alerts('No available ports on '+ ports);
	}
